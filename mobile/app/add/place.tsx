...
const PLACE_TYPES: PlaceType[] = [
  "village", "city", "region", "country", "neighborhood", "building", "site", "other",
];
...
export default function AddPlaceScreen() {
  ...
  const { addPlace, updatePlace, places } = useResearchStore();

  const existing = editId ? places.find((p) => p.id === editId) : undefined;
  ...
  useEffect(() => {
    if (existing) {
      navigation.setOptions({ title: "Modifier le lieu" });
      setName(existing.name);
      setPlaceType(existing.placeType);
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.card}>
        ...
        <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="ex. Tizi Ouzou, Boulogne-Billancourt" placeholderTextColor={Colors.inkMuted} autoFocus={!isEdit} />
